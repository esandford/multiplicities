...
CONTAINS
 ...
 INTEGER, DIMENSION(nmax), INTENT(OUT) :: simcounts, gencounts, transitcounts
 INTEGER, DIMENSION(imax) :: ndetected_table, ngenerated_table, ntransits_table
 ...
 DO WHILE (i .LT. imax)
     ...
     IF ( ndetected .GE. 1 ) THEN
         ...
         ngenerated_table(v) = nplanets
         ntransits_table(v) = ntransiters
 ...
 DO i=1,nmax
     ! how many i-planet systems did you find?
     simcounts(i) = 0
     DO j=1,vmax
 ...
 DO i=1,nmax
     ...
     gencounts(i) = 0
     DO j=1,vmax
         IF( ngenerated_table(j) .EQ. i ) THEN
 ...
 DO i=1,nmax
     ...
     transitcounts(i) = 0
     DO j=1,vmax
         IF( ntransits_table(j) .EQ. i ) THEN
             transitcounts(i) = transitcounts(i) + 1
         END IF
     END DO
 END DO
