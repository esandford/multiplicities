...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumePoisson_"$i".sh"`
  jobname=`echo "resumePoisson_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/poisson/corrR_rayleigh_mcmc"$i".dat"`
