...
for i in `seq $istart $iend`; do
  for j in `seq $jstart $jend`; do
    ...
    scriptname=`echo "./resumefitPoissontofakePoisson_pop"$j"_run"$i".sh"`
    jobname=`echo "resumefitPoissontofakePoisson_pop"$j"_run"$i`
    ...
    # for resume version
    nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/fakes/poisson/fitPoissontofakePoisson/mcmc_pop"$j"_run"$i".dat"`
