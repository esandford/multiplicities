...
for i in `seq $istart $iend`; do
  for j in `seq $jstart $jend`; do
    ...
    scriptname=`echo "./resumefitZipftofakePoisson_pop"$j"_run"$i".sh"`
    jobname=`echo "resumefitZipftofakePoisson_pop"$j"_run"$i`
    ...
    # for resume version
    nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/fakes/poisson/fitZipftofakePoisson/mcmc_pop"$j"_run"$i".dat"`
