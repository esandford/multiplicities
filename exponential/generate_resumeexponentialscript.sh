...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeExponential_"$i".sh"`
  jobname=`echo "resumeExponential_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/exponential/corrR_rayleigh_mcmc"$i".dat"`
