...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeUniform_"$i".sh"`
  jobname=`echo "resumeUniform_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/uniform/corrR_rayleigh_mcmc"$i".dat"`
