...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeDiconstant_"$i".sh"`
  jobname=`echo "resumeDiconstant_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/diconstant/corrR_rayleigh_mcmc"$i".dat"`
