...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeDiexponential_"$i".sh"`
  jobname=`echo "resumeDiexponential_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/diexponential/corrR_rayleigh_mcmc"$i".dat"`
