...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeDizipf_"$i".sh"`
  jobname=`echo "resumeDizipf_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/dizipf/corrR_rayleigh_mcmc"$i".dat"`
