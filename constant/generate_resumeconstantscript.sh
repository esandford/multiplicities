...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeConstant_"$i".sh"`
  jobname=`echo "resumeConstant_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/constant/corrR_rayleigh_mcmc"$i".dat"`
