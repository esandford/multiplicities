...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeDipoisson_"$i".sh"`
  jobname=`echo "resumeDipoisson_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/dipoisson/corrR_rayleigh_mcmc"$i".dat"`
