...
CONTAINS
 ...
 ! =======================================================
 SUBROUTINE model(a_small,a_big,Rpcrit,sigmaR,a_zipf,single_frac,sigmainc,&
                  imax,nmax,kepcounts,&
 ...
 INTEGER :: proceed
 ! File names
 CHARACTER(LEN=200), INTENT(IN) :: trimmedradfile, trimmeddetfile, trimmedcountfile
 
 ! Free parameters
 REAL(8), INTENT(IN) :: a_small, a_big, Rpcrit, sigmaR, a_zipf, single_frac, sigmainc
 REAL(8) :: logRpcrit
 ...
 INTEGER, DIMENSION(nmax), INTENT(OUT) :: simcounts
 INTEGER, DIMENSION(nmax) :: gencounts
 INTEGER, DIMENSION(imax) :: ndetected_table
 INTEGER, DIMENSION(imax) :: ngen_table
 ...
 DO WHILE (i .LT. imax)
     ...
     DO WHILE( ntransiters .LT. 1 )
       ...
       DO WHILE( satisfied .EQ. 0 )
         ...
         ! random # of planets (verified this works!)
           call randomreal(.FALSE.,random_re)
             IF( random_re .LT. single_frac ) THEN
                 nplanets=1
             ELSE
     ...
     IF ( ndetected .GE. 1 ) THEN
         ...
         ! write(*,*) 'detected/transiting/generated = ',ndetected,' / ',ntransiters,' / ',nplanets
         ndetected_table(v) = ndetected
         ngen_table(v) = nplanets
     END IF
 END DO
 vmax = v
 
 ! Compute KS log-like
 ! Rp_accept are radii, not ratio-of-radii. So keplerradii are radii too, in units of Rearth
 call KStwo(imax,ikep,Rp_accept,keplerradii,KSloglike)
 ! spit out the simulated kepler radii
 OPEN(unit=204,file=trimmedradfile)
 DO i=1,imax
 ...
 DO i=1,nmax
     ! how many i-planet systems did you detect?
     simcounts(i) = 0
     DO j=1,vmax
 ...
 DO i=1,nmax
     ! how many i-planet systems did you generate?
     gencounts(i) = 0
     DO j=1,vmax
         IF( ngen_table(j) .EQ. i ) THEN
 ...
 ! spit out simulated *detection* counts
 OPEN(unit=205,file=trimmeddetfile)
 write(205,*) simcounts
 CLOSE(205)

 ! spit out simulated *generated* counts
 OPEN(unit=206,file=trimmedcountfile)
 write(206,*) gencounts
 CLOSE(206)
