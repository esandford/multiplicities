...
for i in `seq $istart $iend`; do
  ...
  scriptname=`echo "./resumeDiuniform_"$i".sh"`
  jobname=`echo "resumeDiuniform_"$i`
  ...
  # for resume version
  nstepsalready=`wc -l "/rigel/home/es3197/multiPlanetDist_fixedForecaster/diuniform/corrR_rayleigh_mcmc"$i".dat"`
